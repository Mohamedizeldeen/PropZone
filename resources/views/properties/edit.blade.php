...
@section('title', 'Edit ' . $property->name . ' - PropZone')

@section('content')
<div class="max-w-2xl mx-auto">
    <!-- Header -->
    <div class="mb-8">
        <a href="{{ route('properties.show', $property) }}" class="text-[#01bbab] hover:text-[#00685f] font-medium mb-2 inline-flex items-center">
        ...
        <p class="mt-2 text-gray-600">Update the details for {{ $property->name }}</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form method="POST" action="{{ route('properties.update', $property) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="type" id="type" required 
                        ...
                        <option value="retail" {{ old('type', $property->type) == 'retail' ? 'selected' : '' }}>Retail Space</option>
            ...
            <div>
                ...
                @if($property->image)
                    <div class="mb-4">
                        ...
                        <img src="{{ asset('storage/' . $property->image) }}" alt="{{ $property->name }}" class="w-32 h-32 object-cover rounded-lg">
                    </div>
                @endif
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-[#01bbab] hover:text-[#00685f] focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-[#01bbab]">
                                <span>{{ $property->image ? 'Replace image' : 'Upload a file' }}</span>
            ...
            <div class="flex justify-end space-x-4 pt-6">
                <a href="{{ route('properties.show', $property) }}" class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition">
                    Cancel
                </a>
